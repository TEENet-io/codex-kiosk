...
test("desktop shortcuts use the console-free VBS launcher", () => {
  assert.match(buildScriptSource, /Codex\.vbs/);
  assert.match(buildScriptSource, /CreateObject\("WScript\.Shell"\)/);
  assert.match(buildScriptSource, /shell\.Run/);
  assert.match(
    installerTemplateSource,
    /Filename: "\{sys\}\\wscript\.exe"; Parameters: """\{app\}\\Codex\.vbs"""/,
  );
  assert.match(
    installerTemplateSource,
    /ValueData: """\{sys\}\\wscript\.exe"" ""\{app\}\\Codex\.vbs"" ""%1"""/,
  );
  assert.ok(verifyScriptSource.includes("'Codex.vbs'"));
});
