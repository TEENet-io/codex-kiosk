...
当前候选：`26.901.51231-b4`，补充命令注册无变化更新的修复，继续屏蔽宠物，沿用 Codex 名称与安装位置。Windows CI 34367229919 已通过。用户清空旧 `.codex`、仅放回 config、model 和 auth 后恢复正常，未依赖 b4；具体触发文件尚未确定，不要求现场继续升级。详见[恢复记录](enterprise-preview.md#2026-09-09-员工清理旧状态后恢复)。尚未通过 ai-env-mgr 发布到员工机队。
