## Codex 26.901.51231-b7（候选）

- 恢复 26.901 简化显示模式下“运行了命令”的展开与收起，可查看原始命令、工作目录、输出及退出状态；保留简洁标题、显示模式和权限配置。固定版本组件回归与包验证要求同步更新。
- Windows 构建 34457431038 已生成 b7 安装器和便携包；英文、中文 bearer 配置的安装后交互验收通过。使用安装后的真实组件与固定数据，原生鼠标验证两种显示模式的展开、收起；返回对话、权限往返、本地项目及宠物输入回归通过。未调用真实模型生成测试命令，现场旧会话仍待用户确认。本次仅提供候选构建，未发布 Release；下载与校验值见 `docs/command-details-26.901.md`。
