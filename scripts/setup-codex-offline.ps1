...
"@
    ...
    AfterSetup = @{
        en = 'After this first setup, open Codex.vbs or the Codex shortcut directly.'
        zh = '首次设置完成后，日常直接打开 Codex.vbs 或 Codex 快捷方式即可。'
    }
    LaunchPrompt = @{
        en = 'Launch Codex now?'
        zh = '现在启动 Codex 吗？'
...
function Register-CodexUrlProtocol {
    ...
    Set-Item -Path $commandRoot -Value "`"$wscriptPath`" `"$LauncherPath`" `"%1`""
}
...
$dailyLauncher = Join-Path $packageRoot 'Codex.vbs'
