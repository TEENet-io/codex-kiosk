...
const CODEX_ONLY_STARTUP_PATCH_MARKER =
  contractPatchMarker('/*codex-offline:codex-only-startup*/');
const CODEX_ONLY_SELECTOR_PATCH_MARKER =
  contractPatchMarker('/*codex-offline:codex-only-selector*/');
const CODEX_ONLY_TRANSITION_PATCH_MARKER =
  contractPatchMarker('/*codex-offline:codex-only-transition*/');
const LOCAL_TASKS_ONLY_COMPOSER_PATCH_MARKER =
  contractPatchMarker('/*codex-offline:local-tasks-only-composer*/');
const CLOUD_TASK_SCHEMA_DISABLED_PATCH_MARKER =
  contractPatchMarker('/*codex-offline:cloud-task-schema-disabled*/');
const CLOUD_TASK_RUNTIME_BLOCKED_PATCH_MARKER =
  contractPatchMarker('/*codex-offline:cloud-task-runtime-blocked*/');
const CLOUD_TASK_LIST_DISABLED_PATCH_MARKER =
  contractPatchMarker('/*codex-offline:cloud-task-list-disabled*/');
const CLOUD_TASK_DETAIL_DISABLED_PATCH_MARKER =
  contractPatchMarker('/*codex-offline:cloud-task-detail-disabled*/');

function patchCodexOnlyLocalTasks(content) {
  const markers = [
    ...
    CLOUD_TASK_LIST_DISABLED_PATCH_MARKER,
    CLOUD_TASK_DETAIL_DISABLED_PATCH_MARKER,
  ];
  if (markers.every(marker => content.includes(marker))) {
    return { content, patched: false, markers: new Set(markers) };
  }

  let next = content;
  const found = new Set(markers.filter(marker => next.includes(marker)));
  const replaceOnce = (needle, replacement, marker) => {
    if (found.has(marker)) return;
    const first = next.indexOf(needle);
    if (first < 0 || next.indexOf(needle, first + needle.length) >= 0) return;
  ...
  replaceOnce(
    ...
    `function v5s({configuredThreadDetailLevel:e,onboardingWorkMode:t,threadDetailLevel:n}){return\`codex\`${CODEX_ONLY_STARTUP_PATCH_MARKER}}`,
    CODEX_ONLY_STARTUP_PATCH_MARKER,
  );
  replaceOnce(
    'codexOnly:h,codexLocalAccessStatus:d.status,productMode:re',
    `codexOnly:!0${CODEX_ONLY_SELECTOR_PATCH_MARKER},codexLocalAccessStatus:d.status,productMode:re`,
    CODEX_ONLY_SELECTOR_PATCH_MARKER,
  );
  replaceOnce(
    'function txc(e,{codexLocalAccessStatus:t,currentMode:n,navigate:r,nextMode:i,startNewConversation:a}){',
    `function txc(e,{codexLocalAccessStatus:t,currentMode:n,navigate:r,nextMode:i,startNewConversation:a}){i=\`codex\`;${CODEX_ONLY_TRANSITION_PATCH_MARKER}`,
    CODEX_ONLY_TRANSITION_PATCH_MARKER,
  );
  replaceOnce(
    'function Uxs({cloudAccess:e,cloudProjectCompatible:t,enforceAccess:n,localAccess:r,preferredLocation:i}){let a={cloud:(n?Wxs(e):null)??(t?null:`project-incompatible`),local:n?Wxs(r):null},o=i;if(a[i]!=null){let e=i===`cloud`?`local`:`cloud`;o=a[e]==null?e:null}return{effectiveLocation:o,unavailableReasons:a}}',
    `function Uxs({cloudAccess:e,cloudProjectCompatible:t,enforceAccess:n,localAccess:r,preferredLocation:i}){let a={cloud:\`codex-only-local-tasks\`,local:n?Wxs(r):null},o=a.local==null?\`local\`:null;return{effectiveLocation:o,unavailableReasons:a}}${LOCAL_TASKS_ONLY_COMPOSER_PATCH_MARKER}`,
    LOCAL_TASKS_ONLY_COMPOSER_PATCH_MARKER,
  );
  replaceOnce(
    ',X({type:Pl(`chatgptWorkCloud`),projectId:Y().min(1).optional()})]),$hl=',
    `])${CLOUD_TASK_SCHEMA_DISABLED_PATCH_MARKER},$hl=`,
    CLOUD_TASK_SCHEMA_DISABLED_PATCH_MARKER,
  );
  replaceOnce(
    'try{if(i.data.target.type===`chatgptWorkCloud`){',
  ...
  if (!found.has(CLOUD_TASK_LIST_DISABLED_PATCH_MARKER)) {
    const listNeedles = [
      ...
      ['let s=e?.enabled!==!1&&n===`chatgpt`,c;', 'let s=!1,c;'],
    ];
    let listPatched = true;
    for (const [needle, replacement] of listNeedles) {
      const index = next.indexOf(needle);
      if (index < 0 || next.indexOf(needle, index + needle.length) >= 0) {
        listPatched = false;
        break;
      }
  ...
  if (!found.has(CLOUD_TASK_DETAIL_DISABLED_PATCH_MARKER)) {
    const detailNeedles = [
      ...
      ['{return{enabled:e!=null,queryFn:async()=>f_.safeGet(`/wham/tasks/{task_id}/turns`,', `{return{enabled:!1,queryFn:async()=>f_.safeGet(\`/wham/tasks/{task_id}/turns\`,`],
    ];
    let detailPatched = true;
    for (const [needle, replacement] of detailNeedles) {
      const index = next.indexOf(needle);
      if (index < 0 || next.indexOf(needle, index + needle.length) >= 0) {
        detailPatched = false;
        break;
...
try {
  ...
  let legacyPluginsPagePatchMigrations = 0;
  {
    ...
    const codexOnlyLocalTasksMarkers = new Set();

    for (const filePath of webviewJsFiles) {
      let content = fs.readFileSync(filePath, 'utf8');
      let changed = false;
      const codexOnlyLocalTasksPatch = patchCodexOnlyLocalTasks(content);
      for (const marker of codexOnlyLocalTasksPatch.markers) {
        codexOnlyLocalTasksMarkers.add(marker);
      }
      if (codexOnlyLocalTasksPatch.patched) {
    ...
    for (const marker of [
      ...
      if (!codexOnlyLocalTasksMarkers.has(marker)) {
        failRequiredPatch(`Could not apply required Codex-only/local-tasks patch: ${marker}`);
      }
    }
  }
