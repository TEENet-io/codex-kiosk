...
foreach ($needle in @(
    ...
    'SetupIconFile={#MySourceRoot}\_internal\app\resources\icon-chatgpt.ico',
    '#define MyInstallerRoot "__INSTALLER_ROOT__"',
    'Name: "zh"; MessagesFile: "{#MyInstallerRoot}\ChineseSimplified.isl"',
    'Filename: "{sys}\wscript.exe"; Parameters: """{app}\Codex.vbs"""; WorkingDir: "{app}"',
    ...
    'URL Protocol',
    'Tasks: codexlinks',
    'ValueData: """{sys}\wscript.exe"" ""{app}\Codex.vbs"" ""%1"""',
    'Filename: "{sys}\WindowsPowerShell\v1.0\powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -File ""{app}\_internal\setup-codex-offline.ps1"" {code:GetSetupCodexArgs} -NoLaunch"; Flags: runhidden waituntilterminated skipifsilent',
    'Filename: "{sys}\wscript.exe"; Parameters: """{app}\Codex.vbs"""; WorkingDir: "{app}"; Description: "{cm:LaunchCodex}"; Flags: nowait postinstall skipifsilent',
...
try {
    ...
    foreach ($needle in @(
        ...
        'After this first setup, open Codex.vbs or the Codex shortcut directly.'
    )) {
    ...
    $dailyLauncherPath = Join-Path $portableRoot 'Codex.vbs'
    $dailyLauncherContent = Get-Content -Path $dailyLauncherPath -Raw
    foreach ($needle in @('_internal\app', 'ChatGPT.exe', 'CODEX_ELECTRON_ENABLE_WINDOWS_COMPUTER_USE', 'WScript.Arguments', 'shell.CurrentDirectory', 'shell.Run')) {
        if (-not $dailyLauncherContent.Contains($needle)) {
            throw "Console-free daily launcher is missing expected marker: $needle"
        }
    }

    $fallbackLauncherPath = Join-Path $portableRoot 'Codex.cmd'
    $fallbackLauncherContent = Get-Content -Path $fallbackLauncherPath -Raw
    foreach ($needle in @('%~dp0_internal\app\ChatGPT.exe', '/D "%~dp0_internal\app"', 'CODEX_ELECTRON_ENABLE_WINDOWS_COMPUTER_USE')) {
        if (-not $fallbackLauncherContent.Contains($needle)) {
            throw "Fallback command launcher is missing expected relative-launch marker: $needle"
        }
    }
...
const CODEX_ONLY_LOCAL_TASKS_PATCH_MARKERS = [
  requiredPatchMarker('/*codex-offline:codex-only-startup*/'),
  requiredPatchMarker('/*codex-offline:codex-only-selector*/'),
  requiredPatchMarker('/*codex-offline:codex-only-transition*/'),
  requiredPatchMarker('/*codex-offline:local-tasks-only-composer*/'),
  requiredPatchMarker('/*codex-offline:cloud-task-schema-disabled*/'),
  requiredPatchMarker('/*codex-offline:cloud-task-runtime-blocked*/'),
  requiredPatchMarker('/*codex-offline:cloud-task-list-disabled*/'),
  requiredPatchMarker('/*codex-offline:cloud-task-detail-disabled*/'),
];
...
const codexOnlyLocalTasksMarkersSeen = new Set();
const codexOnlyLocalTasksResiduals = [];
...
for (const entry of javaScriptEntries) {
  ...
  if (isWebviewAsset) {
    for (const marker of CODEX_ONLY_LOCAL_TASKS_PATCH_MARKERS) {
      if (content.includes(marker)) codexOnlyLocalTasksMarkersSeen.add(marker);
    }
...
const missingCodexOnlyLocalTasksMarkers = CODEX_ONLY_LOCAL_TASKS_PATCH_MARKERS.filter(
  marker => !codexOnlyLocalTasksMarkersSeen.has(marker)
);
if (missingCodexOnlyLocalTasksMarkers.length > 0) {
  throw new Error(
    'Codex-only/local-tasks patch markers are missing from app.asar: ' +
    missingCodexOnlyLocalTasksMarkers.join(', ')
  );
}
if (codexOnlyLocalTasksResiduals.length > 0) {
  throw new Error(
    'ChatGPT/Work or cloud-task surfaces remain enabled in app.asar: ' +
    codexOnlyLocalTasksResiduals.join(', ')
  );
}
