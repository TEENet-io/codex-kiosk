...
function Add-WebGatewayRuntime {
    ...
    $prebuiltRoot = [string]$env:CODEX_OFFLINE_PREBUILT_WEB_GATEWAY
    if (-not [string]::IsNullOrWhiteSpace($prebuiltRoot)) {
        $prebuiltRoot = [System.IO.Path]::GetFullPath($prebuiltRoot)
        Write-BuildTrace "Reusing validated prebuilt Codex Web gateway: $prebuiltRoot"
        & robocopy.exe $prebuiltRoot $destinationRoot /E /COPY:DAT /DCOPY:DAT /R:1 /W:1 /NFL /NDL /NJH /NJS /NP | Out-Null
        if ($LASTEXITCODE -gt 7) {
            throw "robocopy failed to copy the prebuilt Web gateway with exit code $LASTEXITCODE"
        }
    }
    else {
        ...
        node (Join-Path $RepoRoot 'build-tools\materialize-node-modules.mjs') (Join-Path $destinationRoot 'node_modules')
        if ($LASTEXITCODE -ne 0) {
            throw 'Failed to materialize Web gateway node_modules.'
        }
...
$sourceAppPayload = Join-Path $sourceExportRoot 'app'
$stagedAppPayload = Join-Path $internalRoot 'app'
New-Item -ItemType Directory -Force -Path $stagedAppPayload | Out-Null
& robocopy.exe $sourceAppPayload $stagedAppPayload /E /COPY:DAT /DCOPY:DAT /R:1 /W:1 /NFL /NDL /NJH /NJS /NP | Out-Null
if ($LASTEXITCODE -gt 7) {
    throw "robocopy failed to stage the app payload with exit code $LASTEXITCODE"
}
...
$dailyLaunchVbs = @(
    'Option Explicit',
    'Dim shell, fso, packageRoot, appRoot, appExe, arguments, index, value',
    'Set shell = CreateObject("WScript.Shell")',
    'Set fso = CreateObject("Scripting.FileSystemObject")',
    'packageRoot = fso.GetParentFolderName(WScript.ScriptFullName)',
    'appRoot = fso.BuildPath(packageRoot, "_internal\app")',
    'appExe = fso.BuildPath(appRoot, "ChatGPT.exe")',
    'shell.Environment("PROCESS")("CODEX_ELECTRON_ENABLE_WINDOWS_COMPUTER_USE") = "1"',
    ...
    'Next',
    'shell.Run """" & appExe & """" & arguments, 1, False'
)
