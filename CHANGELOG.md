## Codex 26.901.51231-b7（2026-09-10）

- 恢复 26.901 简化显示模式下“运行了命令”的展开与收起，可查看原始命令、工作目录、输出及退出状态；保留简洁标题、显示模式和权限配置。固定版本组件回归与包验证要求同步更新。
- Windows 构建 34457431038 已生成 b7 安装器和便携包；英文、中文 bearer 配置的安装后交互验收通过。使用安装后的真实组件与固定数据，原生鼠标验证两种显示模式的展开、收起；返回对话、权限往返、本地项目及宠物输入回归通过。未调用真实模型生成测试命令，现场旧会话仍待用户确认。
- 用户授权后，发布流程 34459335056 原样发布上述验收资产为 `offline-v26.901.51231-b7`，设为 Latest、非预发布；发布附件 SHA256 与验收包一致。下载与校验值见 `docs/command-details-26.901.md`。

## Codex 26.901.51231-b6（2026-09-10）

- 用户在原问题电脑上确认可用后，正式发布同一份已验收安装包为 `offline-v26.901.51231-b6`；分发附件 SHA256 与验收包一致。
...
- Windows 宠物窗口停用原生透明度淡入淡出，避免重新引入异常分层标志；保留显示、收起及动画。
- 原生窗口标志修复放在随宠物退出的 Node 辅助进程中，限定目标 HWND 归属，失败时保留鼠标穿透。
- Windows 构建 34445177912 的 140 项回归、18 项网关测试、CLI/程序校验和安装器构建通过。同包验收 34446140386 的英文、中文 bearer、中文过期 ChatGPT 凭据三组全部通过，覆盖权限往返、本地项目、返回对话、宠物点击/拖动及透明区域右下角后方按钮。英文首次鼠标程序超时，未改包复测通过；细节、下载与 SHA256 见交付记录。
...
- 修复每日上游探测在 Release 匹配时报 jq 语法错误：通过 `jq --arg` 传递正则，避免反斜杠被解释为 jq 字符串插值；在 jq 内选取首项，保留 API 失败传播。新增实际执行工作流 Bash/jq 的 Ubuntu 回归检查，覆盖已有批次、相邻批次、Issue 去重及恢复关闭。
