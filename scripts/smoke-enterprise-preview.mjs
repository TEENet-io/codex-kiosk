...
try {
  ...
  assert.ok(!/Show or hide pet|Show pet|显示或隐藏(?:虚拟)?宠物|显示宠物/.test(keyboardText), 'pet shortcut absent');
  ...
  await navigate('/settings/pets');
  await delay(1500);
  assert.equal(await evaluate(() => [...document.querySelectorAll('nav[aria-label="个人偏好"] button')].find(button => button.textContent === '外观')?.getAttribute('aria-current')), 'page');
  const petTargets = await (await fetch('http://127.0.0.1:' + port + '/json/list')).json();
  assert.ok(!petTargets.some(item => item.type === 'page' && /avatar/.test(item.url)), 'no pet renderer, including hidden prewarm');
  assert.ok(!/appearance=avatarOverlay/.test(fs.readFileSync(path.join(output, 'desktop.log'), 'utf8')), 'no pet window created during startup');
  if (diagnosticScenario.startsWith('pet-')) assert.equal(JSON.parse(fs.readFileSync(path.join(home, '.codex-global-state.json'), 'utf8'))['electron-avatar-overlay-open'], true, 'saved pet intent preserved without restoring window');
  result.checks.push('pet entry, shortcut, route and startup window disabled without deleting saved state');
