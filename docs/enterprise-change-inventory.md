...
| 默认权限 | 沿用上游或管理员配置 | b6 取消 CLI 强制权限参数和 composer 全访问默认补丁，恢复请求批准选择及 Windows 沙箱设置；保留用户手动选择完全访问，不改管理员配置文件 |
...
- 用户在原问题电脑上确认可用并授权后，已[正式发布 b6](https://github.com/TEENet-io/codex-kiosk/releases/tag/offline-v26.901.51231-b6)，设为 Latest，非预发布。直接下载运行 `codex-only-local-26.901.51231-b6-setup.exe`；退出 Codex 后安装到现有程序目录，无须清空 `.codex`。已安装同一 b6 测试包的用户无需重复安装。
- [发布流程 34449632950](https://github.com/TEENet-io/codex-kiosk/actions/runs/34449632950) 直接取回构建 34445177912 的已验收附件，核对固定 SHA256 后上传草稿，附件齐全才公开。Release API 返回的安装器与便携包摘要均匹配下列值；未重新编译。版本标签指向 `57473bd2cd5532e2990fa73ac21ea642d8a55575`。
